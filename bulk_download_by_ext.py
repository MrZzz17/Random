...
Subcommands:
    ...
    latest     Read a site's RSS feed (cache-busted) and print latest posts
               as a markdown table with --hours / --limit filters. No download.
...
def cmd_latest(url: str, hours: float | None, limit: int) -> int:
    """List newest posts from a site's RSS feed as a markdown table.

    `url` may be either the site root (https://dashablog.tv/) or a direct feed
    URL (https://dashablog.tv/feed/). The request is cache-busted so refreshes
    always hit the origin."""
    import datetime as _dt
    import xml.etree.ElementTree as _ET

    parsed = urlparse(url)
    feed_url = url if parsed.path.rstrip("/").endswith("feed") else (
        url.rstrip("/") + "/feed/"
    )
    sep = "&" if "?" in feed_url else "?"
    feed_url = f"{feed_url}{sep}_={int(_dt.datetime.now().timestamp())}"
    ...
    for it in items:
        ...
        try:
            when = _dt.datetime.strptime(pub, "%a, %d %b %Y %H:%M:%S %z")
        except Exception:
            try:
                when = _dt.datetime.strptime(pub, "%a, %d %b %Y %H:%M:%S GMT").replace(
                    tzinfo=_dt.timezone.utc
                )
            except Exception:
                continue
        age_h = (now - when).total_seconds() / 3600.0
        if hours is not None and age_h > hours:
            continue
        rows.append((age_h, when, title, link))
    ...
    print("| Age | Posted (UTC) | Title | Link |")
    print("|---|---|---|---|")
    for age_h, when, title, link in rows:
        if age_h < 1:
        ...
        print(
            f"| {age} | {when.astimezone(_dt.timezone.utc).strftime('%Y-%m-%d %H:%M')}"
            f" | {title_cell} | {link} |"
        )
    return 0
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        nargs="?",
        default="crawl",
        choices=["scan", "download", "crawl", "dedupe", "clean", "all", "latest"],
        help="scan/download/crawl run the scrape; dedupe and clean are "
             "post-processing; `all` = crawl + dedupe + clean (default: crawl); "
             "`latest` lists newest RSS posts (no download)",
    )
    parser.add_argument("--hours", type=float, help="latest: only show posts newer than N hours")
    parser.add_argument("--limit", type=int, default=50, help="latest: cap on rows shown (default 50)")
    ...
    if args.command == "latest":
        if not args.url:
            print("ERROR: latest requires --url (site root or feed URL)", file=sys.stderr)
            return 2
        return cmd_latest(url=args.url, hours=args.hours, limit=args.limit)
